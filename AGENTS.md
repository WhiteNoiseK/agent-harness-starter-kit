...
| [.claude/commands/harness-verify.md](.claude/commands/harness-verify.md) | **Single source for the stage→agent mapping** (Stage 3→6 orchestration) |
...
| **Phase 3: Implementation** | `{{SRC_ROOT}}/`, `{{TESTS_ROOT}}/`, `implementation_log.md` | `test-writer`, `impl-coder`, `code-reviewer`, `security-reviewer`, `refactor-fixer`, `score-auditor`, `build-error-resolver` | `/harness-verify`, `/tdd`, `/code-review`, `/build-fix` |
...
3. Run the verification gate (/harness-verify):
...
> The responsible agent and permission matrix for each step are single-sourced in [.claude/commands/harness-verify.md](.claude/commands/harness-verify.md) and [docs/_harness/quality-gates.md](docs/_harness/quality-gates.md). This section only summarizes the flow.
