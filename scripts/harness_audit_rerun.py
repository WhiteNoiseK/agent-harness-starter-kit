...
def _is_pytest_cmd(cmd: str) -> bool:
    """Verify the recorded command is a real pytest invocation (blocks gate forgery — MUST NOT CHANGE).

    Checks that the first token (the executable) is python/pytest — a substring check
    would allow false positives like `echo -m pytest 7 passed`, so this is hardened to be token-based.

    Also rejects shell chains/control characters. Because run_cmd reruns with shell=True,
    a chained fake command like `pytest no_such.py & echo 7 passed` could forge the counts
    (making claimed==actual pass). A legitimate pytest_cmd never contains these characters,
    so reject them immediately.

    Allowed: `python -m pytest ...`, `<path>/python.exe -m pytest ...`, leading `pytest ...`.
    Rejected: `echo "7 passed"`, `echo -m pytest 7 passed`, `pytest x & echo 7 passed`.
    """
    # shell chain/control/substitution/redirect characters → score forgery possible → reject
    if re.search(r"[;&|`\n\r]|\$\(|\$\{|[<>]", cmd):
        return False
    try:
    ...
    if re.match(r"^python[0-9.]*(\.exe)?$", base):
        ...
        return (
            len(tokens) >= 3
            and tokens[1] == "-m"
            and tokens[2].strip('"').strip("'") == "pytest"
        )
    # leading executable is pytest
    return bool(re.match(r"^pytest(\.exe)?$", base))
...
def layer2_rerun_verify(stages: list[dict[str, Any]], repo_root: Path) -> list[str]:
    ...
    if mypy_cmd and claimed_mypy is not None:
        ...
        if int(claimed_mypy) != actual_mypy:
            errors.append(f"mypy_errors: claimed={claimed_mypy} actual={actual_mypy}")
    ...
    if ruff_cmd and claimed_ruff is not None:
        ...
        if int(claimed_ruff) != actual_ruff:
            errors.append(f"ruff_errors: claimed={claimed_ruff} actual={actual_ruff}")
...
def layer3_citation_check(stages: list[dict[str, Any]], repo_root: Path) -> list[str]:
    ...
    for st in stages:
        for cite in st.get("citations", []) or []:
            ...
            p = repo_root / rel
            if not p.exists():
                errors.append(f"citation file not found: {rel} (stage={st.get('stage')})")
                continue
...
def main() -> int:
    ...
    if rerun_errors:
        log_block(f"BLOCK: HALLUCINATION — {task_id} claimed values do not match the actual rerun results:")
