...
def check_hook_registered() -> list[str]:
    ...
    if not SETTINGS_JSON.exists():
        problems.append(f"PreToolUse hook not registered: {SETTINGS_JSON} not found (commit guard neutralized)")
        return problems
...
def main() -> int:
    ...
    ap.add_argument(
        "--non-interactive", action="store_true", help="use existing/default values without prompting"
    )
    ap.add_argument(
        "--dry-run", action="store_true", help="print the change plan without actually writing"
    )
    ap.add_argument(
        "--skip-prereq", action="store_true", help="skip the prerequisite check (not recommended)"
    )
    args = ap.parse_args()
    ...
    interactive = (
        not args.non_interactive and not args.dry_run and sys.stdin.isatty()
    )
    print("=" * 60)
    print(
    ...
    if not args.skip_prereq:
        ...
        if problems:
            ...
            print("!" * 60, file=sys.stderr)
            print(
                "  (to skip only the check, use --skip-prereq — at your own risk)", file=sys.stderr
            )
            return 3
        print("[init] prerequisite check passed (hook registered + vendored QG + 4 global agents)")
    else:
