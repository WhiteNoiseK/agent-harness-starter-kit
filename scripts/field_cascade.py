...
def derive_snake(field: str) -> str:
    """CSV field name → derived snake_case (approximation of DB/backend notation)."""
    s = field
    s = s.replace("(m/s)", "").replace("(hub)", "_hub").replace("(dB)", "_db").replace("(db)", "_db")
    s = s.replace("status", "_status").replace("AVL", "_avl")
    s = re.sub(r"(?<=[a-z0-9])(?=[A-Z])", "_", s)      # camelCase boundary
    s = re.sub(r"(?<=[A-Z])(?=[A-Z][a-z])", "_", s)    # consecutive uppercase + lowercase boundary (VShear→V_Shear)
...
def render(
    spec_fields: list[tuple[str, list[str]]], hits: dict[str, list[str]], dep_set: set[str]
) -> str:
    lines = [
        fc._BANNER.replace(
